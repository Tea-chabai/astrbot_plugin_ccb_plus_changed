...
| `/ccbinfo [@]` | 查询某人被C信息（破壁人/次数/注入量/MAX/13水/百合） |
| `/ccbtop` | 被C次数排行 |
| `/ccbvol` | 被注入量排行 |
| `/ccbmax` | 单次最大注入排行（含产生者） |
| `/xnn` | 小南梁榜 |
| `/dj` | 自交（按配置：B=扣B记13水，d=打胶记生命因子） |
| `/djtop` | 自交次数榜（按配置模式显示） |
| `/djmax` | 自交单次最高榜（按配置模式显示） |
| `/bh [@或QQ号]` | 百合互扣；不带目标时为自交 |
| `/bhtop` | 百合被扣榜 |
| `/hn [@或QQ号]` | 喝奈：从目标汲取奶喝，无@时自取其乳（受禁C名单控制） |
| `/hninfo [@]` | 查询泌乳信息（初乳被谁喝了/单次最大/累计） |
| `/hntop` | 泌乳榜：按喂养次数排行 |
| `/ccbclear [@]` | 管理员：清除某人的 CCB 记录 |
| `/ccbnodo [@或QQ号]` | 管理员：禁C名单切换（双向禁C，仍可自交） |
| `/timeclear [@或QQ号]` | 管理员：强制结束指定用户的神罚/昏厥冷却 |

## ⚙️ 配置

AstrBot 配置面板可调：阳痿/昏厥概率与时长、窗口限流（次数/时长）、暴击概率、`dj_mode`（扣B/打胶）、打胶昏厥概率、禁C名单、完整日志开关等。
