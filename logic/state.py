...
def makeit(group_data, target_user_id):
    ...
    if not isinstance(group_data, list):
        return 2  # 脏数据/非列表结构一律按无记录处理，避免迭代崩溃
...
class StateKeeper:
    ...
    def __init__(self, config):
        ...
        self.ban_list = {}
        self.faint_list = {}
        # 关键数值容错：配置缺失/为 None 时回退默认值，避免比较与随机调用抛 TypeError
        self.window = config.get("yw_window") or 60               # 滑动窗口长度（秒）
        self.threshold = config.get("yw_threshold") or 5          # 窗口内最大允许动作次数
        self.ban_duration = config.get("yw_ban_duration") or 300  # 神罚时长（秒）
        self.faint_duration = config.get("faint_ban_duration")    # 昏厥时长（秒，-1/缺失为随机）
        self.faint_random_min = config.get("faint_random_min")    # 昏厥随机最小时长（秒）
        self.faint_random_max = config.get("faint_random_max")    # 昏厥随机最大时长（秒）
        self.yw_prob = config.get("yw_probability") or 0.0        # 阳痿概率
        self.yw_prob_first = config.get("yw_prob_first") or 0.0   # 首次阳痿概率
        self.faint_prob_first = config.get("faint_prob_first") or 0.0  # 首次昏厥概率
        self.faint_prob = config.get("faint_prob") or 0.0         # 昏厥概率
        self.dj_faint_prob = config.get("dj_faint_prob", 0.15) or 0.0  # 打胶/自扣昏厥概率
        self.dj_mode = config.get("dj_mode", "B")                 # 自交模式："B"=扣B，"d"=打胶
        self.crit_prob = config.get("crit_prob") or 0.0           # 暴击概率
        self.selfdo = config.get("self_ccb", False)           # 是否允许自交（0721）
        self.white_list = config.get("white_list") or []      # 禁C名单（缺失/空配置时保持为列表，避免 in 判断异常）
    ...
    def faint_time(self) -> float:
        """昏厥时长：配置固定值或随机区间（缺失/非法配置时自动收敛，避免抛错）"""
        fd = self.faint_duration
        if fd is not None and fd >= 0:
            return fd
        try:
            lo = max(0.0, float(self.faint_random_min))
            hi = max(0.0, float(self.faint_random_max))
    ...
    async def resolve_target(self, event: AstrMessageEvent, messages: str, group_id: str) -> tuple:
        ...
        for seg in event.get_messages():
            # 空 qq 的 At 片段直接跳过，避免把 "None" 当作目标用户
            if isinstance(seg, Comp.At) and getattr(seg, "qq", None) is not None \
                    and str(seg.qq) != str(event.get_self_id()):
