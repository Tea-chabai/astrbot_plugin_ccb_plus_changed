...
def write_json(path: str, data):
    """
    原子写入：先写同目录临时文件并落盘，再 os.replace 覆盖目标。
    即使进程在写入中途被杀/断电，原文件要么保持旧内容、要么已是完整新内容，
    不会出现半截 JSON（read_json 静默回退空表进而覆盖丢失的情况不再可能发生）。
    """
    tmp_path = None
    try:
        ...
        with os.fdopen(fd, "w", encoding="utf-8") as f:
            ...
            f.flush()
            os.fsync(f.fileno())
        os.replace(tmp_path, path)   # 原子替换（Windows 亦支持）
        tmp_path = None
    except Exception as e:
...
class DataStore:
    ...
    def ccb_group_records(self, group_id: str) -> list:
        """ccb.json 某群记录列表。旧数据/脏数据中群内容不是列表时按空处理，避免迭代崩溃"""
        records = self.read_ccb().get(group_id, [])
        return records if isinstance(records, list) else []

    @staticmethod
    def _coerce_ccb_group(group_data) -> list:
        """把某群 ccb 数据归一化为列表（兼容 {用户ID: 记录dict} 的旧结构），保持引用写入"""
        if isinstance(group_data, list):
            return group_data
        if isinstance(group_data, dict):
            return [dict(v, **{a1: k}) for k, v in group_data.items() if isinstance(v, dict)]
        return []
    ...
    def remove_ccb_user(self, group_id: str, user_id: str) -> tuple:
        ...
        if not isinstance(raw, list):
            # 脏数据（如 {用户ID: 记录}）先归一化再操作，避免整组被误清空
            raw = self._coerce_ccb_group(raw)
